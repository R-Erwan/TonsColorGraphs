...
results = dsatur_stats(max_n=20,p=0.5,max_b=4,iteration=1,algo="tons",circular=True,t=4)
print_stats_table(results)
plot_stats(results)
...
results = []
for b in range(2, 6):
    df_b = max_perf_stats_dsatur(node_min=1, p=0.5, b=b, algo="tons", max_avg_time=0.5,circular=True)
    df_b["b"] = b
    results.append(df_b)
